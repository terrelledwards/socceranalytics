...
team = "France"
team_matches = df_match.loc[(df_match["home_team_name"] == team) | (df_match["away_team_name"] == team)]["match_id"].tolist()




df_event = parser.event(3795221)[0]
df_event.info()
...
#This gathers the pass data for the team specified in 'team' in all matches
for i in team_matches:
    df_event = parser.event(i)[0]
    teams = df_event.team_name.unique()
    other_team = teams[0]
    if(teams[0] == team): other_team = teams[1]
   
    df_lineup = parser.lineup(i)
    df_lineup.info() 
   
    sub = df_event.loc[df_event["type_name"] == "Substitution"].loc[df_event["team_name"] == team].iloc[0]["index"]
    pass_england = (df_event.type_name == 'Pass') & (df_event.team_name == team) & (df_event.index < sub) & (df_event.outcome_name.isnull()) & (df_event.sub_type_name != "Throw-in")
    ...
    fig.suptitle("Location of players for " + team + " vs " + other_team, fontsize = 30)
    plt.show()
    ...
    fig.suptitle("Passing Lines for " + team + " vs " + other_team, fontsize=30)
